...
class HybridMemoryMultiFocalPercent(nn.Module):
    def __init__(self, num_features, num_memory, temp=0.05, momentum=0.15, top_percent=0.1):
        super(HybridMemoryMultiFocalPercent, self).__init__()
        self.num_features = num_features
        self.num_memory = num_memory

        self.momentum = momentum
        self.momentum_extract = 0.1
        self.temp = temp
        ...
        self.register_buffer("features", torch.zeros(num_memory, num_features))
        self.register_buffer("features_crop", torch.zeros(num_memory, num_features))
        # self.register_buffer("features_centers", torch.zeros(num_memory, num_features))
        self.register_buffer("labels", torch.zeros(num_memory).long())
    ...
    @torch.no_grad()
    def _update_feature_crop(self, features):
        features = F.normalize(features, p=2, dim=1)
        self.features_crop.data.copy_(features.float().to(self.features.device))

    @torch.no_grad()
    def _update_feature_with_momentum(self, features):
        features = F.normalize(features, p=2, dim=1).float().to(self.features.device)
        self.features = self.momentum_extract * features + (1 - self.momentum_extract) * self.features
        
    @torch.no_grad()
    def _update_feature_crop_with_momentum(self, features):
        features = F.normalize(features, p=2, dim=1).float().to(self.features_crop.device)
        self.features_crop = self.momentum_extract * features + (1 - self.momentum_extract) * self.features_crop
    ...
    def forward(self, results, results_crop, indexes, indexes_gt):
        loss = self.forward_loss(results, indexes) * 0.5
        loss_crop = self.forward_loss_crop(results_crop, indexes_gt) * 0.5

        return loss, loss_crop


    def forward_loss(self, results, indexes):
        inputs = results
        inputs = F.normalize(inputs, p=2, dim=1)
    ...
    def forward_loss_crop(self, results, indexes):
        inputs = results
        inputs = F.normalize(inputs, p=2, dim=1)

        # inputs: B*2048, features: N*2048
        inputs = hm(inputs, indexes, self.features_crop, self.momentum) #B*N, similarity
        inputs /= self.temp
        B = inputs.size(0)
