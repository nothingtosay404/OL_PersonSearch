...
@torch.no_grad()
def process_label_with_context(labels, centers, features, inds, num_classes, diffthres=0.01):
...
#     nbrs = NearestNeighbors(n_neighbors=N_c, algorithm='auto').fit(centers_np)
#     for uid in unique_inds:
#         b = inds == uid
#         tmp_id = b.nonzero()
#         tmp_labels = labels[tmp_id]
...
#         if len(dups) > 0:
#             for dup in dups:
#                 dup_features = features[tmp_id[dup[1]].squeeze(1)]
#                 dup_features_np = dup_features.cpu().numpy()
                
#                 num_neighbors = len(dup[1]) + 2
#                 distances, indices = nbrs.kneighbors(dup_features_np, n_neighbors=num_neighbors)

#                 # Cost matrix for Hungarian assignment
#                 cost_matrix = distances  # Since we want to assign to the nearest centers

#                 # Solve the assignment problem
#                 row_ind, col_ind = linear_sum_assignment(cost_matrix)

#                 # Assign the features to centers ensuring no overlap
#                 assigned_centers = indices[np.arange(len(row_ind)), col_ind]
#                 for i, feature_idx in enumerate(dup[1]):
#                     assigned_center = assigned_centers[i]
#                     diff_distance = abs(distances[i, col_ind[i]] - distances[i, 0])
                    
#                     if diff_distance > diffthres:
#                         labels[tmp_id[feature_idx]] = num_classes
#                         centers = torch.cat((centers, features[tmp_id[feature_idx]]))
#                         # nbrs = NearestNeighbors(n_neighbors=centers_np.shape[0], algorithm='auto').fit(centers_np)
#                         num_classes += 1
#                     else:
#                         labels[tmp_id[feature_idx]] = assigned_center
#                 # tmp_center = centers[dup[0]].cpu().numpy()
#                 # tmp_features = features[tmp_id[dup[1]].squeeze(1)].cpu().numpy()
#                 # # print(tmp_id[dup[1]].squeeze(1), tmp_features.shape)
#                 # sim = np.dot(tmp_center, tmp_features.transpose())
...
def label_generator_dbscan(cfg, features, dist, all_inds, eps, indep_thres):
...
def label_generator_dbscan_context(cfg, features, cuda=True, indep_thres=None, all_inds=None, **kwargs):
    ...
    if len(eps) == 1:
        ...
        # breakpoint()
        features = features.cpu()
        label_generator_dbscan(cfg, features, dist, all_inds, eps, indep_thres)
        print("--------------dist_refine----------------")
        mask = (all_inds[:, None] == all_inds[None, :])
        dist[mask] = 1.0
        ...
        labels_normal, centers, num_classes, indep_thres = label_generator_dbscan(cfg, features, dist, all_inds, eps, indep_thres)
